...
For `add`, `edit`, `see`, `remove`, and `rename`, pass the note **name** only — not `something.md`. The extension comes from `settings.json` (default `md`). `edit` with no name opens the notes folder in your editor. The rest is in `brain --help`.
